...
const { enrichAnalysisResult, computeWork1Metrics, buildNotebookAnalysisData } = require('../analysis/work1-metrics');
...
async function main() {
  ...
  try {
    ...
    charts = [
      { title: '实体类型分布', description: '测试图表1', imageBase64: TINY_PNG, stdout: 'entities: ' + entities.length },
      { title: '关系类型分布', description: '测试图表2', imageBase64: TINY_PNG, stdout: 'relations: ' + relations.length },
    ];
  } catch (e) {
  ...
  console.log(`[6] 验证: 报告库共 ${list.length} 条, 加载 markdown ${loaded.markdown?.length} 字, 图表 ${loaded.charts?.length} 张`);
  console.log('\n✅ 报告存库测试完成');
}
