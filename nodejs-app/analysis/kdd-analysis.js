...
function tokenize(text) {
  return tokenizer.tokenize(text.toLowerCase()).filter((t) => t.length >= 2 && !STOP_WORDS.has(t))
}
...
function mineAssociationRules(labels, featureSets) {
  ...
  return Object.entries(pairCounts)
    .map(([rule, count]) => ({
      ...
      confidence:
        count /
        Math.max(1, Math.min(itemCounts[rule.split(' -> ')[0]] || 1, itemCounts[rule.split(' -> ')[1]] || 1)),
    }))
    .filter((r) => r.count >= 2)
...
function runKddAnalysis(texts, options = {}) {
  ...
  const featureSets = docTokens.map((tokens) => {
    const features = []
    if (tokens.some((t) => /iran|以色列|gaza|hamas|houthi/i.test(t))) features.push('actor_mention')
    if (tokens.some((t) => /strike|attack|ceasefire|sanction|blockade/i.test(t)))
      features.push('action_mention')
    return features
  })
