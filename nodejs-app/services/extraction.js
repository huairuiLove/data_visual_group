...
function normalizeEntity(e) {
  ...
  const validType = SINGLE_ENTITY_TYPES.includes(type)
    ? type
    : config.graph.allowedNodes.includes(type)
      ? type
      : '政治实体'
  return {
    id: name,
...
function normalizeRelation(r) {
  ...
  const validType = SINGLE_RELATION_TYPES.includes(type)
    ? type
    : config.graph.allowedRelationships.includes(type)
      ? type
      : '参与'
  return {
    source,
...
async function extractSingleDetailed(text, meta = {}, provider = 'openai-compatible', options = {}) {
  const prompt = buildSingleArticlePrompt(meta.title || '', meta.summary || '', text, meta.date || '')
  ...
  const response = await chat(
    ...
    provider,
    {
      model: options.model || getModelName(provider),
      temperature: 0.2,
      maxTokens: 4096,
    }
  )
  ...
  const entities = (data.entities || []).map(normalizeEntity).filter(Boolean)
...
async function extractChunkLegacy(text, provider = 'openai-compatible') {
...
2. 实体类型必须从允许列表中选择`
  ...
  const response = await chat(
    ...
    provider,
    { model: getModelName(provider), temperature: 0.2 }
  )
  ...
  const relations = (data.relationships || [])
    ...
    .filter(
      (r) => ids.has(r.source) && ids.has(r.target) && config.graph.allowedRelationships.includes(r.type)
    )
...
async function extractMultiArticleJoint(articles, provider = 'openai-compatible', options = {}) {
