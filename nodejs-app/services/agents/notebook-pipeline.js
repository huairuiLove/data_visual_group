...
const {
  PROFILER_SYSTEM, PLANNER_SYSTEM, CODER_SYSTEM, REVIEWER_SYSTEM,
  buildProfilerUserPrompt, buildPlannerUserPrompt, buildCoderUserPrompt, buildReviewerUserPrompt,
...
async function generateNotebookMultiAgent(analysisResult, options = {}) {
  ...
  const flashModel = options.flashModel || 'deepseek-v4-flash';
  ...
  try {
    const plannerStep = await runAgent(
      ...
      provider,
      flashModel,
      { thinking: true, reasoning_effort: 'medium', maxTokens: 3000 },
    );
  ...
  try {
    const coderStep = await runAgent(
      ...
      provider,
      proModel,
      { thinking: true, reasoning_effort: 'high', maxTokens: 8192 },
    );
