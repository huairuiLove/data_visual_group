...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    return cb(
      new AppError(415, 'UNSUPPORTED_FILE_TYPE', '仅支持 doc、docx、tex、markdown、pdf 和常见文本类文件')
    )
  },
})
...
router.post('/settings/llm', async (req, res) => {
  try {
    ...
    if (!baseURL || typeof baseURL !== 'string') {
      throw new AppError(400, 'MISSING_BASE_URL', 'Base URL 不能为空')
    }
    if (!model || typeof model !== 'string') {
      throw new AppError(400, 'MISSING_MODEL', '模型名称不能为空')
    }
...
router.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const provider = req.body.llmProvider || 'openai-compatible'
